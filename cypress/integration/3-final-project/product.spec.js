...
describe('Final Project - Product Module Tests', ()=> {
    ...
    productTest.forEach(test => {
        it(`Testing Specific Product Info - ${test.tc}`, ()=> {
            inventoryPage.elements.productLink().eq(0).click()
            cy.url().should('be.equal', 'https://www.saucedemo.com/inventory-item.html?id=4')
                productPage.elements.productName().should('have.text', test.name)
                productPage.elements.productPic()
                .invoke("attr", "alt").then((pic) => {
